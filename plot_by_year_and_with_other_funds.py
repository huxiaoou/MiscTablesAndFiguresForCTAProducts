...
# --- load cta index
cta_index_names = ["CTA复合细分策略指数"]
cta_index_df = pd.read_excel(
    os.path.join(input_dir, "朝阳私募指数_1693626441225_0831.xlsx"), sheet_name="中国私募指数",
)[["时间"] + cta_index_names]
cta_index_df.rename(mapper={"时间": "trade_date",
                            "CTA复合细分策略指数": "CTA复合策略指数"}, axis=1, inplace=True)
...
merged_df["CTA复合策略指数"] = cta_index_df["CTA复合策略指数"]
...
plot_df_since_2022 = plot_df.truncate(before=nav_since_date)
...
artist = CPlotLines(
    ...
    line_color=['#DC143C', '#DB7093', '#000080', '#4B0082', '#4169E1', '#4682B4', '#B0C4DE', '#2E8B57'],
    line_style=["-"] * 2 + ["-."] * 5 + ["-"],
...
artist = CPlotLines(
    ...
    line_style=["-"] * 2 + ["-."] * 5 + ["-"],
    xtick_label_size=16, ytick_label_size=16,
    legend_fontsize=16, legend_loc="lower left"
)
artist.plot()
